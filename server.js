...
const sequelize = new Sequelize('kurpek', 'root', 'matthewondahood',{
  host: 'localhost',
  dialect: 'mysql'
})
...
/*
    ...
    app.post('/api/feedback', async (req, res) => {
        ...
        if (!validator.isLength(feedbackInfo.name, { min: 1, max: 80 }))
        ...
        if (!validator.isLength(feedbackInfo.place, { min: 1, max: 80 }))
            validataionError.push('Wrong feedback')
        
        console.log(validator.isLength(feedbackInfo.name, { min: 1, max: 80 })) 
